...
int interface_address(Address *host, char *name, int type)
{
  ...
  for (a = addrs; a != NULL; a = a->ifa_next)
    if (a->ifa_addr && a->ifa_addr->sa_family == family)
    {
      ...
      {
        host->length = type & IPV4 ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6);
        ...
        if (prefix == 0xfe80 && type & LINK_LOCAL)
          goto done;
        if (prefix == 0xfd00 && type & SITE_LOCAL)
          goto done;
      }
    }
done:
