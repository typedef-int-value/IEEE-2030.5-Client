...
char *net_receive(UdpPort *p, int *length)
{
  ...
  *length = recvfrom(p->pe.socket, p->buffer, p->size, 0, (struct sockaddr *)&p->source, &p->source.length);
  p->pe.end = *length < 0;
  // printf ("udp_read %d\n", n); fflush (stdout);
...
int net_send(UdpPort *p, char *buffer, int length, Address *addr)
{
  ...
  return sendto(p->pe.socket, buffer, length, 0, (struct sockaddr *)(addr), addr->length);
}

int net_reply(UdpPort *p, char *buffer, int length)
{
  return sendto(p->pe.socket, buffer, length, 0, (struct sockaddr *)(&p->source), p->source.length);
}
