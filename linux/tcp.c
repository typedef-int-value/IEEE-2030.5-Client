...
#define event_pending(c) (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINPROGRESS)

int accepted(TcpPort *p, Acceptor *a)
{
  Address host;
  host.length = sizeof(Address);
  p->pe.socket = accept(a->pe.socket, (struct sockaddr *)&host, &host.length);
  if (p->pe.socket == -1)
  {
    a->pe.end = 1;
    return 0;
...
void net_connect(void *port, Address *server)
{
  ...
  if (connect(p->pe.socket, (struct sockaddr *)server, server->length) == 0)
  {
    p->pe.status = Connected;
    queue_add(&_active, p);
  }
...
{
  TcpPort *p = port;
  return p->pe.status == Connected ? write(p->pe.socket, data, length) : -1;
}
