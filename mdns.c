...
{
  UdpPort *client = new_udp_port(1500);
  Address addr;
  ...
  while (!truncated(data + 1))
  {
    ...
    switch (*data & 0xc0)
    {
      ...
      case 0xc0: // pointer
        ...
        if (truncated(data))
          return NULL; // exceeds packet length
        if (prev_length == name_length)
          return NULL; // pointer loop with no data
        prev_length = name_length;
        break;
      default:
        printf("dns_name: unknown type in name\n");
        return NULL;
    }
  }
  return NULL;
