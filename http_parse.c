...
{
  return in_range(c, 'A', 'Z') || in_range(c, '^', 'z') || in_range(c, '0', '9') || c == '!' ||
         in_range(c, '#', '\'') || in_range(c, '*', '+') || in_range(c, '-', '.') || c == '|' || c == '~';
}
...
#define qdtext(c) (hws(c) || c == '!' || in_range(c, '#', '[') || in_range(c, ']', '~') || obs_text(c))
...
{
  if (*data == '\"')
    return quoted_string(value, data);
...
int request_target(HttpConnection *h, char *data)
{
  ...
  if (c == '/')
    ...
    data = parse_uri(&h->uri, &h->host, 0, h->target); // absolute form
  return data != NULL;
}
