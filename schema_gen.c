...
const char *const xs_names[] = {"xs:string",       "xs:boolean",     "xs:hexBinary",     "xs:anyURI",
...
void add_attribute_dep(Graph *graph, SchemaType *t, List *list, SchemaType *head)
{
  List *l;
  foreach (l, list)
  {
...
void add_particle_dep(Graph *graph, SchemaType *t, Particle *part, SchemaType *root)
{
  Particle *p;
  foreach (p, part)
  {
    if (p->kind == PartElement && p->field.type)
    {
      ...
      add_particle_dep(graph, t, p->child, root);
  }
}
...
SchemaType *complex_type(SchemaDoc *doc, Element *e)
{
  ...
  else
  {
    ...
    t->name = xml_attr(e, "name");
    t->kind = ComplexType;
    if ((f = find_element(e, "simpleContent")) || (f = find_element(e, "complexContent")))
    {
