...
#define randomizable(type) (type == SE_DERControl || type == SE_TimeTariffInterval || type == SE_EndDeviceControl)
...
void device_response(Stub *device, Stub *event, int status)
{
  ...
  if ((status == EventReceived && ev->responseRequired & 1) || (ev->responseRequired & 2))
  {
...
{
  SE_Event_t *x = resource_data(a->event), *y = resource_data(b->event);
...
EventBlock *new_block(Stub *event, int primacy)
{
  ...
  eb->next = NULL;
  eb->start = ev->interval.start + (rand ? randomize_start(event) : 0);
  eb->end = eb->start + ev->interval.duration + (rand ? randomize_duration(event) : 0);
  eb->primacy = primacy;
  eb->status = 0;
  eb->event = event;
  return eb;
...
{
  EventBlock *prev = (EventBlock *)&s->scheduled, *e = s->scheduled, *next;
  Interval x, y;
  eb->status = Scheduled;
...
EventBlock *schedule_event(Schedule *s, Stub *event, int primacy)
{
  ...
  if (in_range(eb->status, Canceled, Completed))
    return eb;
