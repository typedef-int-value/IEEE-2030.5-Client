...
char *skip_questions(char *data, int count)
{
  ...
  while (count--)
  {
    ok((data = dns_name(name, data)) && !truncated(data + 4));
    data += 4;
  }
  return data;
}
...
void dns_ptr(char *name, char *data)
{
  ...
  char instance[256];
  int length;
  if ((q = find_question(name)) && (data = dns_name(instance, data + 10)))
  {
    ...
    if (data = parse_rr(name, &length, data))
    {
      switch (rr_type(data))
      {
        ...
        case A_RECORD:
          h = find_host(name);
          if (h)
            host_a(h, data, length);
          break;
        case AAAA_RECORD:
          h = find_host(name);
          if (h)
            host_aaaa(h, data, length);
          break;
      }
      data = rr_next(data, length);
    }
...
void dnssd_packet(char *data, int length)
{
  ...
  if ((data = dns_header(&header, data)) && (header.flags & 0xf80f) == 0x8000 &&
...
void dnssd_followup(UdpPort *p)
{
  ...
  if (followup)
    net_send(p, packet, data - packet, &multicast);
}
