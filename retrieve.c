...
#define get_root(conn, obj, type)                                                                                      \
  (se_exists(obj, type##Link) ? get_resource(conn, SE_##type, (obj)->type##Link.href, 0) : NULL)
...
#define get_list_root(conn, obj, type)                                                                                 \
  (se_exists(obj, type##Link) ? get_resource(conn, SE_##type, (obj)->type##Link.href, (obj)->type##Link.all) : NULL)
...
*/
#define get_dep(r, obj, type) new_dep(r, get_root((r)->conn, obj, type), SE_##type##Link_exists)
...
/** @brief Get a subordinate %List resource Stub and make the parent resource a
    ...
    @returns a pointer to the subordinate resource Stub
*/
#define get_list_dep(r, obj, type) new_dep(r, get_list_root((r)->conn, obj, type), SE_##type##Link_exists)
...
void get_seq(Stub *s, int offset, int count)
{
  ...
  if (count)
  {
    ...
    if (count > 255)
      count = 255;
    if (offset)
      sprintf(uri, "%s?s=%d&l=%d", name, offset, count);
    else
      sprintf(uri, "%s?l=%d", name, count);
    http_get(s->conn, uri);
  }
  else
    http_get(s->conn, name);
...
void remove_req(Stub *s, Stub *r)
{
  ...
  if (!r->deps)
    insert_event(r, RESOURCE_REMOVE, 0);
}
...
{
  Stub *head = find_resource(s->base.name), *t = list_remove(head, s);
  if (t)
  {
    if (t != head)
      insert_resource(t);
    ...
    delete_resource(head);
  if (s->moved)
    remove_req(s, s->moved);
  else
    delete_reqs(s);
...
void *insert_stub(List *list, Stub *s, ListInfo *info)
{
  ...
  if (find_by_data(list, s))
    return list;
  while (l)
  {
    Resource *r = l->data;
    if (compare_keys(data, r->data, info) < 0)
      break;
    prev = l, l = l->next;
  }
  n = list_insert(l, s);
...
Stub *get_resource(void *conn, int type, const char *href, int count)
{
  ...
  Uri128 buf;
  Uri *uri = &buf.uri;
  if (!http_parse_uri(&buf, conn, href, 127))
    return NULL;
  if (uri->host)
...
void update_existing(Stub *s, void *obj, DepFunc dep)
{
  ...
  else if (se_event(r->type))
  {
    ...
    memcpy(&ex->EventStatus, &ev->EventStatus, sizeof(SE_EventStatus_t));
    free_se_object(obj, r->type);
  }
  else
    replace_se_object(r->data, obj, r->type);
  dep(s);
  if (!s->flags)
    dep_complete(s);
}
    ...
    case HTTP_RESPONSE:
      switch (status = http_status(conn))
      {
        ...
        case 301:
          ...
          break;
        default:
          if (http_method(conn) == HTTP_GET && (s = find_target(conn)))
          {
            s->status = status;
