...
#define se_post(conn, data, type, href) se_send(conn, data, type, href, HTTP_POST)
#define se_put(conn, data, type, href) se_send(conn, data, type, href, HTTP_PUT)
...
#define link_put(conn, obj, data, type) link_send(conn, obj, data, type, HTTP_PUT)
#define link_post(conn, obj, data, type) link_send(conn, obj, data, type, HTTP_POST)
...
const char *const se_ranges[] = {"application/sep-exi", "application/sep+xml", "application/xml", "application/*",
                                 "*/*"};
...
void *new_conn(int client)
{
  ...
  char *media = se_media == SE_XML ? "application/sep+xml" : "application/sep-exi";
...
void *find_conn(Address *addr)
{
  ...
  foreach (c, connections)
    if (http_client(c) && address_eq(&c->host, addr))
      return c;
  return NULL;
}
...
void *se_send(void *conn, void *data, int type, char *href, int method)
{
  Uri128 buf;
  Uri *uri = &buf.uri;
