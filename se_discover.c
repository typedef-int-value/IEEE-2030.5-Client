...
const char *const service_name[] = {"bill", "dr", "derp", "file", "msg", "ppy",  "rsps",
                                    "tp",   "tm", "upt",  "edev", "mup", "sdev", "smartenergy"};

const int service_types[] = {SE_CustomerAccountList,
                             SE_DemandResponseProgramList,
                             SE_DERProgramList,
                             SE_FileList,
                             SE_MessagingProgramList,
                             SE_PrepaymentList,
                             SE_ResponseSetList,
                             SE_TariffProfileList,
                             SE_Time,
                             SE_UsagePointList,
                             SE_EndDeviceList,
                             SE_MirrorUsagePointList,
                             SE_SelfDevice,
                             SE_DeviceCapability};
...
void *service_connect(Service *service, int secure)
{
  ...
  char *https = NULL;
  if (secure)
    https = txt_value(buffer, service->txt, "https");
  int port = https ? (*https == '\0' ? 443 : atol(https)) : service->port, n_port = htons(port);
