...
void options(int argc, char **argv)
{
  ...
  {
    DerDevice *d;
    if (subtype_query(argv[i], name) || uri_retrieval(argv[i]))
    {
      i++;
      break;
    }
  ...
  {
    uint64_t sfdi;
    const char *const commands[] = {"sfdi",  "edev", "fsa",  "register",  "pin",      "primary",
                                    "all",   "time", "self", "subscribe", "metering", "meter",
                                    "alarm", "poll", "load", "device",    "delete",   "inverter"};
...
void edev_complete(Stub *r)
{
  ...
  if (edev->sFDI == device_sfdi)
  {
    if (test & METER_TEST)
    {
      ...
      se_post(dcap_mup->conn, &mup, SE_MirrorUsagePoint, dcap_mup->base.name);
    }
    if (test & ALARM_TEST)
    {
      if (t = get_list_root(r->conn, edev, LogEventList))
        ...
        test_fail("alarm", "no LogEventList defined for the EndDevice.");
    }
  }
}
...
void get_edev_subs(Stub *edevs)
{
  ...
  foreach (l, edevs->reqs)
  {
    ...
    if (test & INVERTER_CLIENT && e->sFDI != device_sfdi)
      continue;
...
void check_registration(Stub *r)
{
  ...
  if (reg->pIN == 111115)
  {
    ...
    test_fail("registration", "pIN does not match 111115");
}
    ...
    if (test & REGISTER_TEST)
    {
      if (se_exists(e, RegistrationLink))
      {
        if (test & PUT_PIN)
        {
          ...
          get_root(r->conn, e, Registration);
      }
      else
        test_fail("registration", "EndDevice does not contain RegistrationLink.");
    }
  }
}
      ...
      foreach (l, r->reqs)
      {
        ...
        if (d = find_device(&edev->sFDI))
        {
          if (memcmp(edev->lFDI, d->lfdi, 20))
            test_fail("device test", "retrieved LFDI does not match computed LFDI");
        }
        else
...
void dcap(Stub *r)
{
  ...
  if (test & GET_EDEV)
  {
    if (r = get_list_root(r->conn, dcap, EndDeviceList))
      ...
      test_fail("edev", "no EndDeviceListLink in DeviceCapability");
  }
  if (test & GET_SELF)
  {
  ...
  if (d && d->readings)
  {
    ...
    if (!(test & INCLUDE_READINGS))
    {
      ...
      foreach (r, d->readings)
        se_post(d->mup->conn, r->data, SE_MirrorMeterReading, resource_name(d->mup));
    }
...
void post_readings(DerDevice *d)
{
  ...
  foreach (r, d->readings)
  {
    ...
    se_post(d->mup->conn, mmr, SE_MirrorMeterReading, resource_name(d->mup));
  }
