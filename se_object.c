...
*/
#define se_list(type) (se_type_is_a(type, SE_SubscribableList) || se_type_is_a(type, SE_List))
...
*/
#define replace_se_object(dest, src, type) replace_object(dest, src, type, &se_schema)
...
#define compare_uint(a, b, type) (*(type)a < *(type)b) ? -1 : (*(type)a - *(type)b)
...
{
  if (xml)
    output_init(o, &se_schema, buffer, size);
  else
    exi_output_init(o, &se_schema, buffer, size);
}
