...
{
  int c = *data++, x, y;
  if (alpha(c) || in_range(c, '&', ';') || c == '=' || c == '!' || c == '$' || c == '@' || c == '_' || c == '~')
  {
    *(*p)++ = c;
    return data;
  }
...
{
  if (uri->name && *uri->end != '\0')
  { // host name not terminated
...
int write_uri(char *buffer, Uri *uri)
{
  ...
  if (uri->name || uri->host)
  {
    ...
    if (uri->name)
      ...
      n += write_address_port(buffer + n, uri->host);
  }
  if (uri->path)
    n += sprintf(buffer + n, "%s", uri->path);
  if (uri->query)
    n += sprintf(buffer + n, "?%s", uri->query);
  return n;
}
