...
Address *ipv6_address(Address *addr, const char *ip, int port)
{
  ...
  in->sin6_flowinfo = 0;
  in->sin6_scope_id = ipv6_scope(ip);
...
void multicast_join(int socket, const char *addr, int loop)
{
  ...
  if (setsockopt(socket, IPPROTO_IPV6, IPV6_MULTICAST_IF, (void *)&global_if_index, sizeof(int)) < 0)
  ...
  if (setsockopt(socket, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, (void *)&group, sizeof(group)) < 0)
    print_error("setsockopt IPV6_ADD_MEMBERSHIP error");
  // recieve our own multicasts?
  setsockopt(socket, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, (void *)&loop, sizeof(loop));
  // set the time to live
  value = 255;
  if (setsockopt(socket, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, (void *)&value, sizeof(value)) < 0)
