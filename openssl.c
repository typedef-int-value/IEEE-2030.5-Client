...
int verify_peer(int status, X509_STORE_CTX *ctx)
{
  ...
  if (x509 != curr)
    return status; // only check the peer cert
  if (x509 && _verify_peer)
  {
    ...
    cert = p;
    i2d_X509(x509, &p);
    if (!_verify_peer(user, cert, length))
      status = 0;
    free(cert);
  }
  return status;
...
void tls_init(const char *path, const char* private, VerifyFunc verify)
{
  ...
  if ((ret = SSL_CTX_use_PrivateKey_file(ssl_ctx, private, SSL_FILETYPE_PEM)) != 1)
  {
...
#define ssl_pending() (ssl_err == SSL_ERROR_WANT_READ || ssl_err == SSL_ERROR_WANT_WRITE)
