...
{
  if (q->last)
    q->last = q->last->next = item;
  else
    q->first = q->last = item;
}

void _queue_insert(Queue *q, void *item, void *prev, int (*compare)(void *a, void *b))
{
...
void queue_insert(Queue *q, void *item, int (*compare)(void *a, void *b))
{
  List *prev = NULL;
