...
int exi_parse_string(Parser *p, void *value, int n)
{
  ...
  switch (m)
  {
    ...
    default: // literal value encoding
      ...
      if (n)
      { // string is stored in a fixed container
        if (m >= n)
        {
          p->state = PARSE_INVALID;
          return 0;
      ...
      if (!t)
        t = p->local = new_string_table(p->local, name, 8);
...
int exi_parse_sequence(Parser *p, StackItem *t)
{
  ...
  {
    if (p->token)
      p->need_token = 0, p->token--, p->state++;
    else
      return 1;
  }
  return 0;
...
const ParserDriver exi_parser = {exi_parse_start, exi_parse_next,   exi_parse_end,  exi_parse_sequence,
                                 exi_parse_value, exi_parse_simple, exi_parse_done, exi_rebuffer};
