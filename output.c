    ...
    switch (o->state)
    {
      ...
      element_skip:
        ...
        if (!o->n)
          o->n = se->n;
      element_next:
        o->se = pop_element(stack, &o->base);
      output_skip:
        o->code++;
      output_next:
        o->se++;
        o->state = o->se->n ? OUTPUT_ELEMENT : OUTPUT_END;
        break;
      case OUTPUT_ATTRIBUTE:
        if (d->output_event(o, o->se, AT_EVENT))
          o->state++;
        else
          goto full;
      case OUTPUT_ATTR_VALUE:
        ...
        break;
      case OUTPUT_SIMPLE:
        if (d->output_event(o, o->se, SE_SIMPLE))
          o->state++;
        else
          goto full;
      case OUTPUT_VALUE:
        if (d->output_value(o, o->base))
          o->state++;
        else
          goto full;
      case OUTPUT_END:
        if (stack->n)
        {
          t = stack_top(stack);
          se = t->se;
          if (!d->output_event(o, se, EE_EVENT))
            goto full;
