...
#define pack_signed(dest, sx, data) (signed_int(&sx, data) ? *dest = sx, 1 : 0)
#define pack_unsigned(dest, ux, data) (unsigned_int(&ux, data) ? *dest = ux, 1 : 0)
...
int start_tag(Parser *p, const SchemaElement *se)
{
  ...
  switch (parse_token(p))
  {
    ...
    case EMPTY_TAG:
      if (p->empty)
        p->state = PARSE_INVALID;
      else if (!streq(name, p->xml->name))
        break;
      else
      {
        p->empty = p->token;
        p->need_token = 1;
        if (p->empty && se->simple)
          p->state = PARSE_INVALID;
        else
          return 1;
      }
...
{
  return bsearch(&name, schema->names, schema->length, sizeof(char *), compare_names);
}
...
int xml_start(Parser *p)
{
  ...
  while (1)
  {
    ...
    switch (parse_token(p))
    {
      ...
      case START_TAG:
      case EMPTY_TAG:
        if (!(name = find_se_name(p->schema, p->xml->name)))
          goto invalid;
...
const ParserDriver xml_parser = {xml_start,   xml_next,         xml_end,    xml_sequence,
