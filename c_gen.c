...
void print_atomic_typedef(SchemaType *t)
{
  if (streq(t->base, "xs:hexBinary"))
  {
    int n = t->maxLength;
    if (n == 1)
      print("typedef uint8_t SE_%s_t;\n", t->name);
    else
      print("typedef uint8_t SE_%s_t[%d];\n", t->name, n);
  }
  else if (streq(t->base, "xs:string"))
  {
    int n = t->maxLength;
    if (n)
      print("typedef char SE_%s_t[%d];\n", t->name, n);
    else
      print("typedef char *SE_%s_t;\n", t->name);
  }
  else
  {
    const char *type = c_type(t->base);
    if (type)
      print("typedef %s SE_%s_t;\n", type, t->name);
    else
      print("typedef SE_%s_t SE_%s_t;\n", t->base, t->name);
  }
}
...
void print_flags(SchemaType *t, SchemaType *u, SchemaType *head)
{
  ...
  if (u->base)
    print_flags(t, find_by_name(head, u->base), head);
  foreach (l, u->pre)
  {
    f = l->data;
  ...
  foreach (l, u->flags)
    print_flag(l->data, base);
  if (u->post)
  {
    FlagGroup *g = u->post;
    foreach (l, g->flags)
      print_flag(l->data, g->base);
    g->base = 1;
  }
}
...
Flag *add_flag(SchemaType *t, char *name, int min, int count, int bool)
{
  ...
  if (g = find_by_name(flags, name))
  {
    ...
    {
      if (f->min == min && f->count == count && f->bool == bool)
        goto out;
    ...
    switch (p->kind)
    {
      case PartElement:
        ...
        if (!is_pointer(f->prim))
        {
          ...
          count = (p->max > 0 && p->min < p->max) ? bit_count(p->max - p->min) : 0;
          if (count || bool)
...
void compute_flags(List *sorted, SchemaType *head)
{
  ...
  {
    t = l->data;
    if (t->kind == ComplexType && !t->restriction)
    {
      attr_flags(t, head);
...
{
  char *base[] = {"",         "XS_STRING", "XS_BOOLEAN", "XS_HEX_BINARY", "XS_ANY_URI", "XS_LONG", "XS_INT",
                  "XS_SHORT", "XS_BYTE",   "XS_ULONG",   "XS_UINT",       "XS_USHORT",  "XS_UBYTE"};
  int n = type >> 4;
  type &= 0xf;
  if (n)
    print("xs_type(%s,%d)", base[type], n);
  else
    print(base[type]);
}
...
void print_schema_element(SchemaElement *se, char *parent, char *name, char *type, SchemaType *head)
{
  ...
  else
  {
    if (se->unbounded)
      ...
      print(".max=%d, ", se->max);
    if ((se->min < se->max && !is_pointer(se->xs_type)) || (se->simple && se->xs_type == XS_BOOLEAN))
      print(".bit=%d, ", se->bit);
  }
  if (simple)
  {
    ...
    print(".index=%d", get_index(head, type));
  if (se->n)
    print(", .n=%d},\n", se->n);
  else
    print("},\n");
...
TableEntry *te_insert(TableEntry *head, char *name, char *type, SchemaElement *se)
{
...
int productions(TableEntry *entries)
{
  ...
  foreach (e, entries)
  {
    ...
    n++;
    if (first && (se->min < se->max || se->unbounded))
      first = 0;
    else if (se->min)
      break;
  }
  if (!e)
    n++; // reached the end of a sequence
  return n;
}
    ...
    switch (p->kind)
    {
      case PartElement:
        ...
        if (f->prim > 0)
        {
          ...
          se->index = get_index(head, f->type);
...
void print_schema(List *sorted, SchemaDoc *doc)
{
  ...
  foreach (e, doc->elements)
  {
    if (q = find_by_name(qnames, e->name))
    {
      ...
      names = list_insert(names, e->name);
  }
...
void print_list_info(Named *list, SchemaType *schema)
{
  ...
  {
    char *base = find_table_entry(n->name, "subscribable", schema) ? "SubscribableList" : "List";
    l = n->data;
    name = l->data;
    print("  {SE_%s, SE_%s, offsetof(SE_%s_t, %s), SE_%s, {", n->name, base, n->name, name, name);
    foreach (l, l->next)
    {
