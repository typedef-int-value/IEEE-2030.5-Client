...
/** @brief Write a POST request to a buffer and queue the request.
    ...
    See @ref http_send.
*/
#define http_post(conn, buffer, uri) http_send(conn, buffer, uri, HTTP_POST)
/** @brief Write a PUT request to a buffer and queue the request.

    See @ref http_send.
*/
#define http_put(conn, buffer, uri) http_send(conn, buffer, uri, HTTP_PUT)
...
const char *const http_methods[] = {"GET", "PUT", "POST", "DELETE", "HEAD", ""};
...
void http_flush(void *conn)
{
  ...
  SendQueueItem *i;
  while ((i = queue_peek(&h->send)) && conn_write(conn, i->buffer, i->length) > 0)
  {
    if (h->debug)
      print_headers(conn, i->buffer);
    free(queue_remove(&h->send));
  }
  if (queue_empty(&h->send) && h->close)
    conn_close(h);
}
...
{
  HttpConnection *h = conn;
  int n = conn_read(conn, h->buffer + h->length, BUFFER_SIZE - h->length - 1);
  if (n <= 0)
    return n;
  h->length += n;
  h->buffer[h->length] = '\0';
...
int http_receive(void *conn)
{
  ...
  Uri uri;
  int i;
  const char *const headers[] = {"host", "accept", "content-type", "content-length", "connection", "location"};
  ...
  while (1)
  {
    switch (c->state)
    {
      case HTTP_START: // request/status line
        ...
        data = c->data;
        if (*data == '\0')
          break; // allow empty lines to start
        if (c->debug)
          printf("<-- conn = %p ---\n"
                 "%s\r\n",
                 c, data);
        ...
        if (c->client)
        {
          if ((data = token_sp(&text, data)) && streq(text, c->version) // status line
              && (data = token_sp(&text, data)) && number(&c->status, text) && c->status <= 999 &&
              (r = dequeue_request(c)) && request_target(c, r->uri))
          {
            c->context = r->context;
        ...
        else if ((data = token_sp(&method, data)) && (data = token_sp(&target, data)) && request_target(c, target) &&
                 streq(data, c->version))
        { // request line
          ...
          case '\0': // end of headers
            ...
            if ((c->method == HTTP_RESPONSE &&
                 ((c->header & HTTP_CONTENT_LENGTH && !c->end) ||
                  ((c->status >= 100 && c->status <= 199) || c->status == 204 || c->status == 304))) ||
            ...
            else
            {
              c->state++; // HTTP_DATA
              if (!c->end)
                c->close = 1; // close-delimited message
            }
