...
{
  return alpha(c) || c == ':' || c == '_' || in_range(c, 0xc0, 0xd6) || in_range(c, 0xd8, 0xf6) ||
         in_range(c, 0xf8, 0x2ff) || in_range(c, 0x370, 0x37d) || in_range(c, 0x37f, 0x1fff) ||
         in_range(c, 0x200c, 0x200d) || in_range(c, 0x2070, 0x218f) || in_range(c, 0x2c00, 0x2fef) ||
         in_range(c, 0x3001, 0xd7ff) || in_range(c, 0xf900, 0xfdcf) || in_range(c, 0xfdf0, 0xfffd) ||
         in_range(c, 0x10000, 0xeffff);
}

int name_char(int c)
{
  return c == '-' || c == '.' || digit(c) || name_start(c) || c == 0xb7 || in_range(c, 0x300, 0x36f) ||
...
int xml_char(int c)
{
  return ws(c) || in_range(c, 0x20, 0xd7ff) || in_range(c, 0xe000, 0xfffd) || in_range(c, 0x10000, 0x10ffff);
}
...
char *xml_reference(char **ptr, char *data)
{
  ...
  *end++ = '\0';
  ...
  {
    data++;
    data = *data == 'x' ? hex_number(&ref, data + 1) : number(&ref, data);
    return data && *data == '\0' ? *ptr = utf8_encode(*ptr, ref), end : NULL;
  }
      ...
      case 2: // "<" (tag)
        switch (c)
        {
          ...
          default:
            ...
            if (strchr(next, '>'))
              return (p->data = xml_tag(p, data)) ? p->state = 0, p->token : XML_INVALID;
            goto incomplete;
        }
        break;
